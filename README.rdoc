...
You can store your s3 secret access key id and secret access key as environment variables, and access them like this:

  @store = ThreeStore.new(ENV['S3_KEY'], ENV['S3_SECRET'], 'my-bucket')

Or you can create a yaml file to store them in:
...
Options to pass to +store_on_s3+: 
  ...
  * :access (optional) - Default access is private, but can be set to 'public-read' and maybe even 'public-write'.  For more information, look in the Amazon S3 docs: http://aws.amazon.com/documentation/
